...
public class ChangePassword extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (!newPass.equals(confirm)) {
            session.setAttribute("error", "Confirm new password invalid!");
            response.sendRedirect("change-password");
        } else {
            try {
                id = Integer.parseInt(id_raw);
                if (!cusDB.checkOldPass(id, oldPass)) {
                    session.setAttribute("error", "Old password invalid!");
                    response.sendRedirect("change-password");
                } else {
                    cusDB.changePassword(id, newPass);
